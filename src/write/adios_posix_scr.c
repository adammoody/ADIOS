...
static int adios_posix_scr_initialized = 0;
...
void adios_posix_scr_init (const PairStruct * parameters
                      ,struct adios_method_struct * method
                      )
{
    struct adios_POSIX_data_struct * p = 0;

    if (!adios_posix_scr_initialized)
    {
        adios_posix_scr_initialized = 1;
    }
...
int adios_posix_scr_open (struct adios_file_struct * fd
...
START_TIMER (ADIOS_TIMER_AD_OPEN);
    ...
    switch (fd->mode)
    {
        ...
        {
            // ATM: tell SCR about the subfile, ask SCR where to put it
            char subfile_name_scr[SCR_MAX_FILENAME];
            SCR_Route_file(subfile_name, subfile_name_scr);

            p->b.f = open (subfile_name_scr, O_RDONLY | O_LARGEFILE);
            if (p->b.f == -1)
            {
        ...
        case adios_mode_write:
        {
            ...
            if (p->group_comm != MPI_COMM_SELF)
            {
                if (p->rank == 0)
                {
                    ...
                    // ATM: don't create directories in SCR
            ...
            p->b.f = open (subfile_name_scr, O_WRONLY | O_CREAT | O_TRUNC | O_LARGEFILE
                                       ,  S_IRUSR | S_IWUSR
                                        | S_IRGRP | S_IWGRP
                                        | S_IROTH | S_IWOTH
                            );
            if (p->b.f == -1)
            {
                fprintf (stderr, "adios_posix_scr_open failed for "
                                 "base_path %s, subfile name %s\n"
                        ,method->base_path, subfile_name_scr
                        );
            ...
            if (p->group_comm != MPI_COMM_SELF && p->g_have_mdf)
            {
                if (p->rank == 0)
                {
                    // ATM: tell SCR about the metadata file, ask SCR where to put it
                    char mdfile_name_scr[SCR_MAX_FILENAME];
                    SCR_Route_file(mdfile_name, mdfile_name_scr);

                    p->mf = open (mdfile_name_scr, O_WRONLY | O_CREAT | O_TRUNC | O_LARGEFILE
                                  ,  S_IRUSR | S_IWUSR
                                   | S_IRGRP | S_IWGRP
                                   | S_IROTH | S_IWOTH
                             );
                    if (p->mf == -1)
                    {
                        fprintf (stderr, "adios_posix_scr_open failed for "
                                ...
                                ,method->base_path, mdfile_name_scr
                                );
        ...
        case adios_mode_update:
        {
            ...
            if (!p->file_is_open) 
            {
                ...
                p->b.f = open (subfile_name_scr, O_RDWR | O_LARGEFILE);
                if (p->b.f == -1)
                {
                    old_file = 0;
                    p->b.f = open (subfile_name_scr,  O_WRONLY | O_CREAT | O_LARGEFILE,
                                  S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
                    if (p->b.f == -1)
                    {
                        fprintf (stderr, "adios_posix_scr_open failed to create  file %s\n" ,subfile_name_scr);
                ...
                p->rank == 0)
            {
                ...
                p->mf = open (mdfile_name_scr, O_WRONLY | O_TRUNC | O_LARGEFILE
                        , S_IRUSR | S_IWUSR
                        | S_IRGRP | S_IWGRP
                        | S_IROTH | S_IWOTH
                        );
                if (p->mf == -1)
                {
                    p->mf = open (mdfile_name_scr, O_WRONLY| O_CREAT | O_LARGEFILE
                            , S_IRUSR | S_IWUSR
                            | S_IRGRP | S_IWGRP
                            | S_IROTH | S_IWOTH
                            );
...
enum BUFFERING_STRATEGY adios_posix_scr_should_buffer (struct adios_file_struct * fd
                                                  ,struct adios_method_struct * method
                                                  )
{
...
void adios_posix_scr_write (struct adios_file_struct * fd
...
void adios_posix_scr_get_write_buffer (struct adios_file_struct * fd
                                  ,struct adios_var_struct * v
                                  ,uint64_t * size
                                  ,void ** buffer
                                  ,struct adios_method_struct * method
...
void adios_posix_scr_read (struct adios_file_struct * fd
                      ,struct adios_var_struct * v
                      ,void * buffer
...
static void adios_posix_scr_write_pg (struct adios_file_struct * fd
                                 ,struct adios_method_struct * method
                                 )
{
    struct adios_POSIX_data_struct * p = (struct adios_POSIX_data_struct *)
...
static void adios_posix_scr_write_index (struct adios_file_struct * fd
        ,struct adios_method_struct * method
        ,char * buffer
        ,uint64_t buffer_size
        )
...
static void adios_posix_scr_do_read (struct adios_file_struct * fd
        ,struct adios_method_struct * method
        )
{
    struct adios_POSIX_data_struct * p = (struct adios_POSIX_data_struct *)
...
void adios_posix_scr_buffer_overflow (struct adios_file_struct * fd, 
                                  struct adios_method_struct * method)
{
    ...
    */
    ...
    adios_posix_scr_write_pg (fd, method); // Buffered vars written here
    STOP_TIMER (ADIOS_TIMER_IO);

    STOP_TIMER (ADIOS_TIMER_AD_OVERFLOW);
}

void adios_posix_scr_close (struct adios_file_struct * fd
                       ,struct adios_method_struct * method
                       )
{
    struct adios_POSIX_data_struct * p = (struct adios_POSIX_data_struct *)
    ...
    switch (fd->mode)
    {
        case adios_mode_write:
        {
            ...
            START_TIMER (ADIOS_TIMER_IO);
            adios_posix_scr_write_pg (fd, method); 
            STOP_TIMER (ADIOS_TIMER_IO);

            // Note: adios_posix_scr_write_pg() sets the fd->current_pg->pg_start_in_file
            ...
            START_TIMER (ADIOS_TIMER_IO);
            adios_posix_scr_write_index (fd, method, buffer, buffer_offset); 
            STOP_TIMER (ADIOS_TIMER_IO);
        ...
        case adios_mode_read:
        {
            ...
            adios_posix_scr_do_read (fd, method);
            struct adios_var_struct * v = fd->group->vars;
            while (v)
            {
                v->data = v->adata = 0;
...
void adios_posix_scr_finalize (int mype, struct adios_method_struct * method)
{
    struct adios_POSIX_data_struct * p = (struct adios_POSIX_data_struct *)
                                                          method->method_data;
    if (p->file_is_open) {
    ...
    if (adios_posix_scr_initialized)
        adios_posix_scr_initialized = 0;
}

void adios_posix_scr_end_iteration (struct adios_method_struct * method)
{
}

void adios_posix_scr_start_calculation (struct adios_method_struct * method)
{
}

void adios_posix_scr_stop_calculation (struct adios_method_struct * method)
{
}
